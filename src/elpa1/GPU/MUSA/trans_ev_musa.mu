...
__global__ void cuda_scale_qmat_double_complex_kernel(muDoubleComplex *q, muDoubleComplex *tau, const int ldq, const int l_cols) {
    
    double one = 1.0;
    double zero = 0.0;
    muDoubleComplex c_one = make_muDoubleComplex(one, zero);
	...
	//q[index].y = -q[index].x * tau[1].y + q[index].y * (one - tau[1].x);
      ...
      q[index] = muCmul(q[index], muCsub(c_one, tau[1]));
    }
}

extern "C" void musa_scale_qmat_double_complex_FromC(int ldq, int l_cols, double _Complex *q_dev, double _Complex *tau_dev, musaStream_t  my_stream){

  muDoubleComplex* q_casted = (muDoubleComplex*) q_dev;
  muDoubleComplex* tau_casted = (muDoubleComplex*) tau_dev;
...
__global__ void cuda_scale_qmat_float_complex_kernel(muFloatComplex *q, muFloatComplex *tau, const int ldq, const int l_cols) {
    
    float one = 1.0f;
    float zero = 0.0f;
    muFloatComplex c_one = make_muFloatComplex(one, zero);
    ...
    if (col < l_cols) {
      //  q[index].x = q[index].x * (one - tau[1].x);
      //  q[index].y = q[index].y * (zero - tau[1].y);
      q[index] = muCmulf(q[index], muCsubf(c_one, tau[1]));
    }

}

extern "C" void musa_scale_qmat_float_complex_FromC(int ldq, int l_cols, float _Complex *q_dev, float _Complex *tau_dev, musaStream_t  my_stream){
  muFloatComplex* q_casted = (muFloatComplex*) q_dev;
  muFloatComplex* tau_casted = (muFloatComplex*) tau_dev;
...
#endif
  musaError_t gpuerr = musaGetLastError();
  if (gpuerr != musaSuccess){
    printf("Error in executing cuda_scale_qmat_float_complex_kernel: %s\n",musaGetErrorString(gpuerr));
  }
}
